...
$repository = "plan-04/p23-vision-language-search"
$service = "ai-04-p23-vision-language-search"
$stack = $service
...
if (-not (Test-Path (Join-Path $root "docker\production.Dockerfile"))) { throw "Production Dockerfile not found." }
if (-not $ValidateAws -and -not $Apply) {
    Write-Host "Local AWS release preflight passed. Add -ValidateAws or -Apply."
    exit 0
}
...
if ($LASTEXITCODE -ne 0) {
    & $aws ecr create-repository @awsCommon --repository-name $repository `
        --image-tag-mutability IMMUTABLE --image-scanning-configuration scanOnPush=true `
        --tags Key=Path,Value=ai-engineer Key=Plan,Value=04 Key=Project,Value=23 *> $null
...
& $aws cloudformation deploy @awsCommon --template-file $template --stack-name $stack `
    --parameter-overrides "ServiceName=$service" "ImageIdentifier=$image" `
    --capabilities CAPABILITY_NAMED_IAM --no-fail-on-empty-changeset `
    --tags Path=ai-engineer Plan=04 Project=23 Environment=portfolio
Assert-ExitCode "App Runner CloudFormation deployment failed."

$serviceUrl = (& $aws cloudformation describe-stacks @awsCommon --stack-name $stack `
    --query "Stacks[0].Outputs[?OutputKey=='ServiceUrl'].OutputValue | [0]" --output text).Trim()
