...
$env:RUFF_CACHE_DIR = Join-Path $env:TEMP "ruff-p23"

Push-Location $root
try {
    & $python scripts\build_qualification_bundle.py
    if ($LASTEXITCODE -ne 0) { throw "Qualification bundle generation failed." }
    & $python -m ruff check assistant backend multimodal scripts tests
    if ($LASTEXITCODE -ne 0) { throw "Ruff lint failed." }
    & $python -m ruff format --check assistant backend multimodal scripts tests
    ...
    Write-Host "Project 23 quality gate passed."
}
