...
$repository = "plan-04/p22-document-extraction"
$service = "ai-04-p22-document-extraction"
$stack = $service
...
if ($LASTEXITCODE -ne 0) {
    & $aws ecr create-repository @awsCommon --repository-name $repository `
        ...
        --tags Key=Path,Value=ai-engineer Key=Plan,Value=04 Key=Project,Value=22 *> $null
...
& $aws cloudformation deploy @awsCommon --template-file $template --stack-name $stack `
    ...
    --tags Path=ai-engineer Plan=04 Project=22 Environment=portfolio
...
Write-Host "Workbench: $serviceUrl/app/"
